...
export const generateFingerprint = (audioData) => {
  ...
  for (
    ...
    i < audioData.length - WINDOW_SIZE;
    i += Math.floor(WINDOW_SIZE * (1 - OVERLAP)), chunkCount++
  ) {
    ...
    if (chunkCount % logInterval === 0 || chunkCount === totalChunks - 1) {
      console.log(
        `Fingerprint generation: ${Math.round(
          (chunkCount / totalChunks) * 100
        )}% complete (${chunkCount}/${totalChunks} chunks)`
      );
    }
  }
...
export const matchFingerprints = (queryFingerprints, dbFingerprints) => {
  console.log(
    `Starting fingerprint matching: ${queryFingerprints.length} sample fingerprints vs ${dbFingerprints.length} song fingerprints`
  );
  const startTime = Date.now();
  ...
  console.log(
    `Performed ${actualComparisons} actual comparisons out of ${totalComparisons} potential comparisons (${Math.round(
      (actualComparisons / totalComparisons) * 100
    )}% efficiency)`
  );
  ...
  timeDiffs.forEach((data, diff) => {
    ...
    for (let i = 0; i < sortedMatches.length; i++) {
      if (
        i > 0 &&
        sortedMatches[i].queryIndex === sortedMatches[i - 1].queryIndex + 1 &&
    ...
    const weightedScore = data.score * 0.4 + data.count * 0.3 + maxConsecutive * 0.3;
    ...
    if (weightedScore > bestScore) {
      ...
      const rawConfidence = matchRatio * 0.5 + consecutiveWeight * 0.3 + avgMatchQuality * 0.2;

      // Apply sigmoid-like function with proper bounds
      const normalizedConfidence = Math.min(
        0.99,
        Math.max(0, rawConfidence < 0.5 ? rawConfidence * 0.8 : 0.4 + (rawConfidence - 0.5) * 1.2)
      );
  ...
  if (bestDiff) {
    console.log(
      `Best match: offset=${bestDiff.offset}, count=${
        bestDiff.count
      }, confidence=${bestDiff.confidence.toFixed(4)}`
    );
    return [bestDiff];
  } else {
